...
function scrollHeader() {
    ...
    if (this.scrollY >= 50) {
        ...
        header.style.boxShadow = '0 4px 12px rgba(30, 64, 175, 0.1)';
    } else {
        header.style.boxShadow = '0 2px 8px rgba(30, 64, 175, 0.08)';
    }
}
...
galleryItems.forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (img) {
            ...
            // Add styles (Updated hover color to blue theme var(--primary-color))
...
function addHoverEffect() {
    const interactiveElements = document.querySelectorAll('.btn, .categoria__card, .beneficio__item, .nav__link');
...
// ==================== CONSOLE MESSAGE ====================
console.log('%c🧠 Thiago Ribeiro - Psicólogo Clínico 🧠', 'color: #1e40af; font-size: 20px; font-weight: bold;');
console.log('%cSite desenvolvido com foco em excelência e performance.', 'color: #4b5563; font-size: 12px;');
...
document.querySelectorAll('.footer__social-link, .contato__info-link').forEach(link => {
    const icon = link.querySelector('i');
    if (icon) {
        const platform = icon.className.split('fa-')[1] || "Link";
